...
import { createDocumentAndChunks, listDocuments } from "../../../lib/knowledgeBase";
...
export async function POST(request: NextRequest) {
    ...
    let body: CreateDocumentDto | null = null;
    ...
    try {
        ...
        if (contentType.includes("multipart/form-data")) {
            ...
            if (typeof requestBody === "string" && requestBody.trim()) {
                body = JSON.parse(requestBody) as CreateDocumentDto;
            }
            ...
            body = (await request.json()) as CreateDocumentDto;
        }
    } catch (error) {
...
export async function GET(request: NextRequest) {
    try {
        ...
        const entries = await listDocuments(documentType);
